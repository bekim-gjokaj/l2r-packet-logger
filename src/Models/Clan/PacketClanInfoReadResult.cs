...
namespace Kamael.Packets.Clan
{
    public class PacketClanInfoReadResult : IL2RPacket
    {
        public PacketClanInfoReadResult(L2RPacket packet)
        {
            ...
            SquireCount = packet.ReadByte();
            packet.Skip(1);
            GuardCount = packet.ReadByte();
            packet.Skip(1);
            KnightCount = packet.ReadByte();
            packet.Skip(1);
            CaptainCount = packet.ReadByte();
            packet.Skip(1);
            ElderCount = packet.ReadByte();
            packet.Skip(1);
            RequireApproval = packet.ReadByte();
            LevelRequirement = packet.ReadByte();
            CastleOwned = packet.ReadUInt32();
            packet.Skip(1);
            FortressOwned = Convert.ToString(packet.ReadUInt32());
            RedStarStone = packet.ReadUInt16();
